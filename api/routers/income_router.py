...
def create_income():
    ...
    return jsonify(IncomeRead.model_validate(new_income).model_dump(), 201)
...
def get_income(income_id):
    ...
    if not income:
        return jsonify({"error": _("INCOME_NOT_FOUND"), "details": _("None")}), 404
    return jsonify(IncomeRead.model_validate(income).model_dump())
...
def update_income(income_id):
    ...
    income = db.query(Income).filter(Income.id == income_id).first()
    if not income:
        return Response._error(_("INCOME_NOT_FOUND"),_("None"), 404)
...
def delete_income(income_id):
    ...
    income = db.query(Income).filter(Income.id == income_id).first()
    if not income:
        return Response._error(("INCOME_NOT_FOUND"),_("None"), 404)
    db.delete(income)
    db.commit()
    return Response._error(_("INCOME_DELETED"),_("NONE"), 204)
