...
from schemas.income_category_schema import IncomesCategoryCreate, IncomesCategoryUpdate
...
def create():
    ...
    try:
        data = IncomesCategoryCreate.model_validate(request.json)
...
def update(id):
    db: Session = next(get_db())
    try:
        data = IncomesCategoryUpdate.model_validate(request.json)
    except ValidationError as e:
        return Response._error(_("VALIDATION_ERROR"), e.errors(), 400, name)
    try:
